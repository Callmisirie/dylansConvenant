...
import { NewsletterSignupModel, NewsletterModel } from "../models/Newsletter.js";
...
router.post("/", async (req, res) => {
    ...
    if (email) {
        try {
            const newsletter = new NewsletterSignupModel({email});
            newsletter.save();
...
router.post("/update", async (req, res) => {
    ...
    let newsletterSignups = await NewsletterSignupModel.find({})
    ...
    console.log(newsletterSignups);
        ...
        if (title && message) {
            try {
                ...
                const update = new NewsletterModel(updateDetails);
